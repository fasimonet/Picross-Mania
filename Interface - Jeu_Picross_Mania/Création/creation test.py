...
# Cette boucle parcourt la grille centrale en longueur.
        for y in range (NBR_CARRE_CENTRAL):
            MATRICE_GAUCHE = zeros((5,3)) # défini la matrice de gauche avec 5 lignes et 3 colonnes
            print(MATRICE_GAUCHE)
            # initialise l'index de la grille de la matrice gauche vide
...
# Cette boucle parcourt la grille centrale en hauteur.
            for x in range(NBR_CARRE_CENTRAL):
                ...
                """MATRICE_GAUCHE[] = 1 # il est ici question d'ajouter un nouvel index initialiser à 1"""

                if MATRICE_GRILLE[CASE_X, CASE_Y] == 1 and PRECEDENT ==1: #si comporte un 1 dans la matrice ET si la case précédente est noire
                    MATRICE_GAUCHE = (MATRICE_GAUCHE) + 1 # ajouter 1 à l'index actuel
                ...
                if MATRICE_GAUCHE == []  # si la matrice est vide alors on assigne à la matrice [0]
                    MATRICE_GAUCHE = 0

# Cette boucle parcourt la grille centrale en longueur.
        for x in range (NBR_CARRE_CENTRAL):
            MATRICE_HAUT = zeros((3,5))
            print (MATRICE_HAUT)
            PRECEDENT = MATRICE_GAUCHE[CASE_X, CASE_Y-1]  # défini PRECEDENT comme la case precedente a CASE_X
            PRECEDENT = 0 #intialise la variable PRECEDENT à 0 soit comme une case vide

            for x in range(NBR_CARRE_CENTRAL):
                if MATRICE_HAUT[CASE_X, CASE_Y] == 1 and PRECEDENT =0:
                    """"MATRICE_HAUT[,] =1 # c'est a dire on ajoute un element dans la grille"""

                if MATRICE_HAUT[CASE_X, CASE_Y] == 1 and PRECEDENT =1:
                    MATRICE_HAUT= (MATRICE_HAUT) + 1

                PRECEDENT= CASE_Y

            if MATRICE_HAUT == [] #si la matrice est vide
                MATRICE_HAUT = [0] # alors on assigne a la matrice la valeure [0]
...
print Matrice_grille_haut1_niveau1
