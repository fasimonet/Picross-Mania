...
# Affichage des différentes grilles
        ...
        Afficher_Indic_Gauche(FENETRE, BLUE_PEGASUS, X_ORIGINE, Y_ORIGINE, 3, 5, MAT_GAUC)

        Afficher_Indic_Haut(FENETRE, BLUE_PEGASUS, X_ORIGINE,Y_ORIGINE, 3, 5, MAT_HAUT)

        if allclose(MAT_CENT, MATRICE_GRILLE):
                continuer_grille = 0
                animation_son = 1
                animation = 1
...
# Remise a zéro de la liste des picross en cours
                for i in range (len(PICROSS_EN_COURS_NIV1)):
                    if PICROSS_EN_COURS_NIV1[i] == True :
                        ...
                        SOMME_PICROSS_COMPLETS = somme_elements_liste(PICROSS_COMPLETS_NIV1)
                        print(PICROSS_EN_COURS_NIV1)
                        print(PICROSS_COMPLETS_NIV1)
                        print(SOMME_PICROSS_COMPLETS)
        else :
                continuer_grille = 1
#-------------------------------------------------------------------------------
...
# Cette boucle affiche un carré de couleur prédéfinie lorsque la matrice aux coordonnées [x,y] est égale à 1 càd dans le cas d'une case noircie.
                    ...
                    Afficher_Carre(FENETRE, VERT_PIN, X_ORIGINE, Y_ORIGINE, x, y)
...
# Si un de ces événements est de type MOUSEBUTTONDOWN, càd un clic quel qu'il soit alors :
            if event.type == MOUSEBUTTONDOWN:
                ...
                if event.pos[0] >= X_ORIGINE and event.pos[0] <= X_ORIGINE + NBR_CARRE_CENTRAL_5x5 * TAILLE_CARRE and event.pos[1] >= Y_ORIGINE and event.pos[1] <= Y_ORIGINE + TAILLE_CARRE * NBR_CARRE_CENTRAL_5x5:
...
# Affichage d'une couleur de fond
        ...
        MATRICE_GRILLE = zeros((COLONNES,LIGNES))
